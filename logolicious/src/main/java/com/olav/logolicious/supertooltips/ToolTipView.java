...
package com.olav.logolicious.supertooltips;
...
public class ToolTipView extends LinearLayout implements ViewTreeObserver.OnPreDrawListener, View.OnClickListener {
    ...
    private void applyToolTipPosition() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
            ...
            mTopPointerView.setVisibility(showBelow ? GONE : VISIBLE);
            mBottomPointerView.setVisibility(showBelow ? VISIBLE: GONE);
        }

        int toolTipViewY;
        if (showBelow) {
            toolTipViewY = toolTipViewBelowY - 100;
        } else {
            toolTipViewY = toolTipViewAboveY + 100;
        }

        if (mToolTip.getAnimationType() == ToolTip.AnimationType.NONE) {
            ViewHelper.setTranslationY(this, toolTipViewY);
            ViewHelper.setTranslationX(this, toolTipViewX - 120);
        } else {
