...
import com.olav.logolicious.cropper.cropwindow.handle.Handle;
import com.olav.logolicious.cropper.util.AspectRatioUtil;
...
public class CropOverlayView extends View {
     ...
     */
    private Paint mCornerPaint, mCornerCirPaint, mPixelPaint, mPixelText;
    ...
    private void init(Context context) {
        ...
        mCornerCirPaint = PaintUtil.newCornerCirclePaint(context);
        mPixelPaint = PaintUtil.newPixelCounterPaint(context);
        mPixelText = PaintUtil.newPixelCounterTextPaint(context);
...
//        //Draw Pixel Counter
//        int bW = GlobalClass.origBitmapwidth; //Bitmap width
//        int bH = GlobalClass.origBitmapheight; //Bitmap height
//        //Orig image ratio
//        float ratio = (float)bW / (float)bH;
//        //get the crop area
//
//        Rect actualRect = ActivityImageCropNew.getActualRect();
//        int sW = actualRect.right - actualRect.left;
//        int sH = actualRect.bottom - actualRect.top;
//
//        String pixelCounterText = "W: " + sW + ", " + " H: " + sH;
//        canvas.drawText(pixelCounterText, l, t - 20, mPixelText);
        ...
        canvas.drawCircle(l, t, 10, mCornerCirPaint);
        // Top right
        ...
        canvas.drawCircle(r, t, 10, mCornerCirPaint);
        ...
        canvas.drawCircle(l, b, 10, mCornerCirPaint);
...
//        System.out.println("LEFT " + LEFT + " TOP " + TOP + " BOTTOM " + BOTTOM + " RIGHT " + RIGHT);
    ...
    private void onActionDown(float x, float y) {
        ...
        float w = mBorderPaint.getStrokeWidth();
        final float left = Edge.LEFT.getCoordinate() + w;
        final float top = Edge.TOP.getCoordinate() + w;
        final float right = Edge.RIGHT.getCoordinate() - w;
        final float bottom = Edge.BOTTOM.getCoordinate() - w;

        mPressedHandle = HandleUtil.getPressedHandle(x, y, left, top, right, bottom, mHandleRadius);
        // corner pressed detection
        int detectSize = 50;
...
//            Log.i("","xxx Not-Center Touch");
            ...
            ((x < (right + detectSize) && x > (right - detectSize)) && (y > (top + detectSize) && y < (bottom + detectSize))
                    ...
                    ||
                    // Left Line
                    ((x < (left + detectSize) && x > (left - detectSize)) && (y > (top + detectSize) && y < (bottom - detectSize)))
                    ||
                    // Top Line
