...
{
    value = cleanValue(value, 180) / 180 * (float) Math.PI;
    if (value == 0){
        return;
    }
...
public static PorterDuffColorFilter adjustLightness(int progress) {
//	http://stackoverflow.com/questions/9175088/adjusting-lightness-using-colormatrix
    if(progress>0)
    {
        int value = (int) progress*255/100;
        return new PorterDuffColorFilter(Color.argb(value, 51, 51, 255), Mode.SRC_ATOP);
    } else {
...
public static void adjustSaturation(ColorMatrix cm, float value) {
    ...
    if (value == 0) {
        return;
                               ...
                               //3 *
    float x = 1+((value > 0) ? 1 * value / 50 : value / 50);
    // orig
//    float lumR = 0.3086f;
//    float lumG = 0.6094f;
//    float lumB = 0.0820f;
